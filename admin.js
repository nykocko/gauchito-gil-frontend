document.addEventListener("DOMContentLoaded", () => {
  ...
  const renderAdminPosts = (posts) => {
      ...
      row.innerHTML = `
                ...
                <td>
                    <span class="px-2 py-1 rounded-full text-xs font-semibold ${
                      post.approved ? "bg-green-600" : "bg-red-600"
                    }">
                        ${post.approved ? "Sí" : "No"}
                    </span>
                </td>
                ...
                <td class="whitespace-nowrap">
                    <button data-id="${post._id}" data-approved="${
        post.approved
      }" class="toggle-approval-btn bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm mr-2">
                        ${post.approved ? "Desaprobar" : "Aprobar"}
                    </button>
                    <button data-id="${
                      post._id
                    }" class="edit-post-btn bg-yellow-500 hover:bg-yellow-600 text-gray-900 px-3 py-1 rounded text-sm mr-2">
                        Editar
                    </button>
                    <button data-id="${
                      post._id
                    }" class="delete-post-btn bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm">
                        Eliminar
                    </button>
                </td>
            `;
    ...
    adminPostsTableBody
      .querySelectorAll(".toggle-approval-btn")
      .forEach((btn) => {
  ...
  const handleToggleApproval = async (e) => {
    ...
    if (
      !confirm(
        `¿Estás seguro de que quieres ${
          newApprovedStatus ? "aprobar" : "desaprobar"
        } este post?`
      )
    ) {
      return;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Error al cambiar estado de aprobación."
        );
      }

      showAdminToast(
        `Post ${newApprovedStatus ? "aprobado" : "desaprobado"} con éxito.`
      );
  ...
  const handleEditPost = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || "No se pudo cargar el post para editar."
        );
      }
        ...
        post.comments.forEach((comment) => {
          ...
          commentElement.innerHTML = `
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-1">Por: ${
                              comment.name
                            } el ${new Date(comment.date).toLocaleDateString(
            "es-AR"
          )}</p>
                        </div>
                        <button data-post-id="${post._id}" data-comment-id="${
            comment._id
          }" class="delete-comment-btn text-red-300 hover:text-red-500 text-sm ml-4 focus:outline-none">
  ...
  const handleDeleteComment = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/admin/posts/${postId}/comments/${commentId}`,
        {
          method: "DELETE",
          headers: getAuthHeaders(),
        }
